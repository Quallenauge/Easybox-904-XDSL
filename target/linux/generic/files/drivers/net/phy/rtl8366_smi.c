...
int rtl8366_smi_read_reg(struct rtl8366_smi *smi, u32 addr, u32 *data)
{
	...
	if (smi->mdio_enabled) {
		...
		smi->mii_bus->write(smi->mii_bus, RTL_MDIO_PHYID,
				    MDC_MDIO_START_REG, MDC_MDIO_START_OP);
		...
		*data = smi->mii_bus->read(smi->mii_bus, RTL_MDIO_PHYID,
...
#endif
	/* Direct MDIO implementation here */
	if (smi->mdio_enabled) {
		np = of_parse_phandle(smi->parent->of_node, "mdio", 0);
		if (np) {

...
#endif  /* CONFIG_CRYPTO_DEV_POLL_DMA */
...
extern void deu_dma_priv_init(void);
#endif
...
int __init lqdeu_async_des_init (void)
{
    ...
    crypto_init_queue(&des_queue->list, 100);
    spin_lock_init(&des_queue->lock);
   
    tasklet_init(&des_queue->des_task, process_queue, 0);
 
    des_queue->des_pid = kernel_thread(des_crypto_thread, des_queue,

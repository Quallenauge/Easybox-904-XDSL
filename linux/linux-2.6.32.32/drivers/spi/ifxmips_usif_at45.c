...
ifx_dataflash_status(struct ifx_dataflash *spi)
{
    ...
    ifx_usif_spiLock(spi->handler);
    ifx_usif_spiTx(spi->handler, &cmd, 1);
    ifx_usif_spiRx(spi->handler, &status, 1);
    ifx_usif_spiUnlock(spi->handler);
    return status;
}
                 ...
                 u8 * wbuf, u32 wcnt, u8 * rbuf, u32 rcnt)
{
    ...
    /* Command and addr */
    ifx_usif_spiTx(spi->handler, cmd, cmd_size);

    /* Dummy cycles */
    if (dummy_cycles > 0 && dummy_cycles < IFX_MAX_DUMMY_CYCLES) {
        ifx_usif_spiTx(spi->handler, buf, dummy_cycles);
    }

    /* TX */
    if (wcnt != 0) {
        ifx_usif_spiTx(spi->handler, wbuf, wcnt);
    }

    if (rcnt != 0) {
        ifx_usif_spiRx(spi->handler, rbuf, rcnt);
    }
...
#define IFX_DATA_FLASH_MODE                IFX_USIF_SPI_MODE_0
#define IFX_DATA_FLASH_PRIORITY            IFX_USIF_SPI_PRIO_LOW
#define IFX_DATA_FLASH_FRAGSIZE            560
#define IFX_DATA_FLASH_MAXFIFOSIZE         32
#define IFX_DATA_FLASH_DRV_VERSION         "0.0.1"
#define IFX_DATA_FLASH_CS                  IFX_USIF_SPI_CS1
#define IFX_DATA_FLASH_NAME                "ifx_dataflash"
#ifdef CONFIG_USE_EMULATOR
#define IFX_DATA_FLASH_BAUDRATE            10000                        /*  10K Hz */
#else
#define IFX_DATA_FLASH_BAUDRATE            2000000                       /*  2 MHz */
#endif
...
static INLINE int 
ifx_data_flash_cs_handler(u32 csq, IFX_USIF_SPI_CS_DATA_t cs_data)
{
...
ifx_data_flash_register(char *dev_name) 
{
    ...
    usif_cfg.baudrate     = IFX_DATA_FLASH_BAUDRATE;
    usif_cfg.csset_cb     = ifx_data_flash_cs_handler;
    usif_cfg.cs_data      = IFX_DATA_FLASH_CS;
    usif_cfg.fragSize     = IFX_DATA_FLASH_FRAGSIZE;
    usif_cfg.maxFIFOSize  = IFX_DATA_FLASH_MAXFIFOSIZE;
    usif_cfg.spi_mode     = IFX_DATA_FLASH_MODE;
    usif_cfg.spi_prio     = IFX_DATA_FLASH_PRIORITY;
    return ifx_usif_spiAllocConnection(dev_name, &usif_cfg);
}

static int
ifx_data_flash_probe(IFX_USIF_SPI_HANDLE_t handler)
{
    u8 cmd = OP_READ_ID;
    u8 manufacturer_id, device_id1, device_id2;
    
    ifx_usif_spiLock(handler);
    /* Send the request for the part identification */
    ifx_usif_spiTx(handler, &cmd, sizeof(cmd));
    /* Now read in the manufacturer id bytes */
    do {
        ifx_usif_spiRx(handler, &manufacturer_id, 1);
    ...
    ifx_usif_spiRx(handler, &device_id1, 1);
    /* Now read in the second device id byte */
    ifx_usif_spiRx(handler, &device_id2, 1);
    ifx_usif_spiUnlock(handler);
...
ifx_dataflash_init(void)
{
    ...
    printk("ATMEL45XX serial flash driver version %s\n", IFX_DATA_FLASH_DRV_VERSION);
    return status;
}
...
ifx_dataflash_exit(void)
{
    ...
    ifx_usif_spiFreeConnection(ifx_data_flash.handler);

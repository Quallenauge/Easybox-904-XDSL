 ...
 * ####################################
 */
#ifndef CONFIG_IFX_A2_DESC_NUM
  #define CONFIG_IFX_A2_ETH_RX      64
  #define CONFIG_IFX_A2_ETH_TX      64
  #define CONFIG_IFX_A2_ATM_RX      128
  #define CONFIG_IFX_A2_ATM_TX      64
#endif
/*!
 ...
 */
static int dma_rx_descriptor_length = CONFIG_IFX_A2_ATM_RX;    /*  Number of descriptors per DMA RX channel        */
/*!
  \brief Number of descriptors per DMA TX channel
 */
static int dma_tx_descriptor_length = CONFIG_IFX_A2_ATM_TX;    /*  Number of descriptors per DMA TX channel        */
/*!
...
static int eth_rx_descriptor_length = CONFIG_IFX_A2_ETH_RX;

static int eth_tx_descriptor_length = CONFIG_IFX_A2_ETH_TX;
...
MODULE_PARM(eth_rx_descriptor_length, "i");
MODULE_PARM_DESC(eth_rx_descriptor_length, "Number of descriptor assigned to ETH RX channel (>16)");
MODULE_PARM(eth_tx_descriptor_length, "i");
MODULE_PARM_DESC(eth_tx_descriptor_length, "Number of descriptor assigned to ETH TX channel (>16)");
...
#if !defined(ENABLE_NAPI) || !ENABLE_NAPI
  static void do_eth_tasklet(unsigned long);
#endif
static irqreturn_t eth_irq_handler(int, void *);
static INLINE void eth_signal(unsigned int, int);
...
static INLINE int init_eth_priv_data(void);
static INLINE void clear_eth_priv_data(void);
static INLINE void init_eth_tables(void);
...
static struct net_device *g_eth_net_dev = NULL;

#if !defined(ENABLE_NAPI) || !ENABLE_NAPI
  DECLARE_TASKLET(g_eth_tasklet, do_eth_tasklet, 0);
#endif

static int g_eth_prio_queue_map[8] = {3, 2, 1, 0, 0, 0, 0, 0};

static unsigned char g_my_ethaddr[8] = {0};
...
static struct net_device_stats *eth_get_stats(struct net_device *dev)
{
    ...
    priv->stats.rx_dropped    = priv->rx_dropped + ETH_MIB_TABLE->erx_dropdes_pdu + priv->enets_igdrop;
...
static int eth_open(struct net_device *dev)
{
...
#if defined(ENABLE_HW_FLOWCONTROL) && ENABLE_HW_FLOWCONTROL
    {
        ...
        if ( (priv->fc_bit = netdev_register_fc(dev, eth_xon)) == 0 ) {
            err("Hardware Flow Control register fails");
        }
    }
#endif

    IFX_REG_W32_MASK(0, 0x0F, MBOX_IGU0_IER);
...
{
    IFX_REG_W32_MASK(0x0F, 0, MBOX_IGU0_IER);
...
#if defined(ENABLE_NAPI) && ENABLE_NAPI
static ifx_eth_fw_poll_ret_t eth_poll(struct net_device *dev, int work_to_do, int *work_done)
{
    struct eth_priv_data *priv = ifx_eth_fw_netdev_priv(dev);
    int qid;
    ...
    while ( *work_done < work_to_do ) {
        for ( qid = 0; qid < 4; qid++ )
            if ( ERX_DMA_CHANNEL_CONFIG(qid)->vlddes != 0 )
                break;
        if ( qid == 4 )
            break;

        reg_desc = priv->rx_desc[qid][priv->rx_desc_pos[qid]];
        if ( reg_desc.own || !reg_desc.c )
            continue;

        skb = priv->rx_skb[qid][priv->rx_desc_pos[qid]];

        new_skb = alloc_skb_rx();
        if ( new_skb != NULL ) {
#if defined(ENABLE_LESS_CACHE_INV) && ENABLE_LESS_CACHE_INV
            if ( reg_desc.byteoff + reg_desc.datalen - 4 > LESS_CACHE_INV_LEN )
                dma_cache_inv((unsigned long)skb->data + LESS_CACHE_INV_LEN, reg_desc.byteoff + reg_desc.datalen - 4 - LESS_CACHE_INV_LEN);
#endif

            skb_reserve(skb, reg_desc.byteoff);
            skb_put(skb, reg_desc.datalen - 4 /* CRC */);

            dump_skb(skb, DUMP_SKB_LEN, (char *)__func__, 0, qid, 0);

            //  parse protocol header
            skb->dev = g_eth_net_dev;
            skb->protocol = eth_type_trans(skb, skb->dev);

            netif_receive_skb(skb);

            priv->rx_packets++;
            priv->rx_bytes += reg_desc.datalen;

            reg_desc.dataptr = ((unsigned int)new_skb->data >> 2) & 0x0FFFFFFF;
            priv->rx_skb[qid][priv->rx_desc_pos[qid]] = new_skb;
        }
        else {
            priv->rx_dropped++;
        }
        ...
        priv->rx_desc[qid][priv->rx_desc_pos[qid]] = reg_desc;
        if ( ++(priv->rx_desc_pos[qid]) == eth_rx_descriptor_length )
            priv->rx_desc_pos[qid] = 0;
    ...
    //  clear interrupt
    IFX_REG_W32_MASK(0, 0x0F, MBOX_IGU0_ISRC);
    //  no more traffic
    for ( qid = 0; qid < 4; qid++ )
        if ( ERX_DMA_CHANNEL_CONFIG(qid)->vlddes != 0 )
            break;
    if ( qid == 4 ) {   //  no valid descriptor
        ifx_eth_fw_poll_complete(dev);
        IFX_REG_W32_MASK(0, 0x0F, MBOX_IGU0_IER);
        return IFX_ETH_FW_POLL_COMPLETE;
    }

    return IFX_ETH_FW_POLL_CONTINUE;
}
#endif
...
static int eth_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    desc_base = -1;
    full = 1;
    if ( priv->tx_desc[qid][priv->tx_desc_pos[qid]].own == 0 ) {
        desc_base = priv->tx_desc_pos[qid];
        if ( ++(priv->tx_desc_pos[qid]) == eth_tx_descriptor_length )
            priv->tx_desc_pos[qid] = 0;
        if ( priv->tx_desc[qid][priv->tx_desc_pos[qid]].own == 0 )
            full = 0;
    }

    //  TODO: check whether implement backpressue

    if ( desc_base < 0 )
        goto ETH_HARD_START_XMIT_FAIL;
    desc = &priv->tx_desc[qid][desc_base];

    /*  write back to physical memory   */
    dma_cache_wback((unsigned long)skb->data, skb->len);

    /*  free previous skb   */
    if ( priv->tx_skb[qid][desc_base] != NULL )
        dev_kfree_skb_any(priv->tx_skb[qid][desc_base]);
    priv->tx_skb[qid][desc_base] = skb;
    ...
    dump_skb(skb, DUMP_SKB_LEN, (char *)__func__, 0, qid, 1);
...
#if !defined(ENABLE_NAPI) || !ENABLE_NAPI
static void do_eth_tasklet(unsigned long arg)
{
    struct eth_priv_data *priv = ifx_eth_fw_netdev_priv(g_eth_net_dev);
    unsigned int work_to_do = 32;
    ...
    //  interface down
    if ( !netif_running(g_eth_net_dev) )
        return;
    ...
    //  next round
    tasklet_schedule(&g_eth_tasklet);
}
#endif
...
static irqreturn_t eth_irq_handler(int irq, void *dev_id)
{
    ...
    isr = IFX_REG_R32(MBOX_IGU0_ISR) & IFX_REG_R32(MBOX_IGU0_IER);
    IFX_REG_W32(isr, MBOX_IGU0_ISRC);

    if ( (isr & 0x0F) ) {
        //  RX
        IFX_REG_W32_MASK(0x0F, 0, MBOX_IGU0_IER);
#if defined(ENABLE_NAPI) && ENABLE_NAPI
        //netif_rx_schedule(g_eth_net_dev);
        ifx_eth_fw_poll_schedule(g_eth_net_dev);
#else
        tasklet_schedule(&g_eth_tasklet);
#endif
    }
    ...
    if ( (isr & 0xF0) ) {
        //  TX
        IFX_REG_W32_MASK(0xF0, 0, MBOX_IGU0_IER);
        netif_wake_queue(g_eth_net_dev);
    }

    return IRQ_HANDLED;
}

static INLINE void eth_signal(unsigned int queue, int is_tx)
{
    int count = 1000;

    if ( is_tx ) {
        while ( MBOX_IGU4_ISR_ISR(queue + 4) && count > 0 )
            count--;
        *MBOX_IGU4_ISRS = MBOX_IGU4_ISRS_SET(queue + 4);
    }
    else {
        while ( MBOX_IGU4_ISR_ISR(queue) && count > 0 )
            count--;
        *MBOX_IGU4_ISRS = MBOX_IGU4_ISRS_SET(queue);
    }

    ASSERT(count > 0, "queue = %u, is_tx = %d, MBOX_IGU4_ISR = 0x%08x", queue, is_tx, IFX_REG_R32(MBOX_IGU4_ISR));
}
...
static int proc_read_mib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + off + len,    "ATM Firmware\n");
    ...
    len += sprintf(page + off + len,    "ATM Driver\n");
    ...
    eth_get_stats(g_eth_net_dev);
    len += sprintf(page + off + len,    "ETH            Hardware ,  Firmware ,   Driver\n");
    len += sprintf(page + off + len,    "  rx_packets:    N/A    , %10u, %10u\n",                       ETH_MIB_TABLE->erx_pass_pdu,    priv->rx_packets);
    len += sprintf(page + off + len,    "  rx_bytes:      N/A    , %10u, %10u\n",                       ETH_MIB_TABLE->erx_pass_bytes,  priv->rx_bytes);
    len += sprintf(page + off + len,    "  rx_errors:  %10u,    N/A    ,    N/A\n", priv->enets_igerr);
    len += sprintf(page + off + len,    "  rx_dropped: %10u, %10u, %10u\n",         priv->enets_igdrop, ETH_MIB_TABLE->erx_dropdes_pdu, priv->rx_dropped);
    len += sprintf(page + off + len,    "  tx_packets:    N/A    , %10u, %10u\n",                       ETH_MIB_TABLE->etx_total_pdu,   priv->tx_packets);
    len += sprintf(page + off + len,    "  tx_bytes:      N/A    , %10u, %10u\n",                       ETH_MIB_TABLE->etx_total_bytes, priv->tx_bytes);
    len += sprintf(page + off + len,    "  tx_errors:  %10u,    N/A    , %10u\n", priv->enetf_egcol,                                    priv->tx_errors);
    len += sprintf(page + off + len,    "  tx_dropped: %10u,    N/A    , %10u\n", priv->enetf_egdrop,                                   priv->tx_dropped);
...
static int proc_write_mib(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    if ( stricmp(p, "clear") == 0 || stricmp(p, "clear all") == 0
        ...
        memset(ETH_MIB_TABLE, 0, sizeof(*ETH_MIB_TABLE));
        memset(&priv->stats, 0, (unsigned int)&priv->rx_desc - (unsigned int)&priv->stats);
    }
...
static INLINE int ifx_atm_version(char *buf)
{
    ...
    len += ifx_drv_ver(buf + len, "ATM+ETH (A2)", IFX_ATM_VER_MAJOR, IFX_ATM_VER_MID, IFX_ATM_VER_MINOR);
    len += sprintf(buf + len, "    ATM (A2) firmware version %d.%d\n", major, minor);
...
static INLINE int init_eth_priv_data(void)
{
    ...
    size = 4 * eth_rx_descriptor_length * sizeof(struct rx_descriptor) + DESC_ALIGNMENT;
    ...
    p = (void *)((((unsigned int)p + DESC_ALIGNMENT - 1) & ~(DESC_ALIGNMENT - 1)) | KSEG1);
    for ( i = 0; i < 4; i++ )
        priv->rx_desc[i] = &((volatile struct rx_descriptor *)p)[i * eth_rx_descriptor_length];

    size = 4 * eth_rx_descriptor_length * sizeof(struct sk_buff *) + DESC_ALIGNMENT;
    p = kzalloc(size, GFP_KERNEL);
    if ( p == NULL )
        return IFX_ERROR;
    dma_cache_wback_inv((unsigned long)p, size);
    ...
    p = (void *)((((unsigned int)p + DESC_ALIGNMENT - 1) & ~(DESC_ALIGNMENT - 1)) | KSEG1);
    for ( i = 0; i < 4; i++ )
        priv->rx_skb[i] = &((struct sk_buff **)p)[i * eth_rx_descriptor_length];

    size = 4 * eth_tx_descriptor_length * sizeof(struct tx_descriptor) + DESC_ALIGNMENT;
    p = kzalloc(size, GFP_KERNEL);
    if ( p == NULL )
        return IFX_ERROR;
    dma_cache_wback_inv((unsigned long)p, size);
    ...
    p = (void *)((((unsigned int)p + DESC_ALIGNMENT - 1) & ~(DESC_ALIGNMENT - 1)) | KSEG1);
    for ( i = 0; i < 4; i++ )
        priv->tx_desc[i] = &((volatile struct tx_descriptor *)p)[i * eth_tx_descriptor_length];

    size = 4 * eth_tx_descriptor_length * sizeof(struct sk_buff *) + DESC_ALIGNMENT;
    p = kzalloc(size, GFP_KERNEL);
    if ( p == NULL )
        return IFX_ERROR;
    dma_cache_wback_inv((unsigned long)p, size);
    ...
    p = (void *)((((unsigned int)p + DESC_ALIGNMENT - 1) & ~(DESC_ALIGNMENT - 1)) | KSEG1);
    for ( i = 0; i < 4; i++ )
        priv->tx_skb[i] = &((struct sk_buff **)p)[i * eth_tx_descriptor_length];
    ...
    rx_desc.datalen = RX_DMA_CH_AAL_BUF_SIZE;
    for ( j = 0; j < 4; j++ ) {
        for ( i = 0; i < eth_rx_descriptor_length; i++ ) {
            skb = alloc_skb_rx();
            if ( skb == NULL )
                return IFX_ERROR;
            rx_desc.dataptr = ((unsigned int)skb->data >> 2) & 0x0FFFFFFF;
            priv->rx_desc[j][i] = rx_desc;
            priv->rx_skb[j][i] = skb;
        }
    }
...
static INLINE void clear_eth_priv_data(void)
{
    ...
    for ( j = 0; j < 4; j++ ) {
        if ( priv->tx_skb[j] == NULL )
            continue;
        for ( i = 0; i < eth_tx_descriptor_length; i++ ) {
            skb = priv->tx_skb[j][i];
            if ( skb )
                dev_kfree_skb_any(skb);
        }
    ...
    if ( priv->tx_skb_base != NULL )
        kfree(priv->tx_skb_base);

    if ( priv->tx_desc_base != NULL )
        kfree(priv->tx_desc_base);

    if ( priv->rx_skb_base != NULL )
        kfree(priv->rx_skb_base);

    if ( priv->rx_desc_base != NULL )
        kfree(priv->rx_desc_base);
}

static INLINE void init_eth_tables(void)
{
    struct eth_priv_data *priv = ifx_eth_fw_netdev_priv(g_eth_net_dev);
    struct wrx_dma_channel_config rx_cfg = {0};
    struct wtx_dma_channel_config tx_cfg = {0};
    int i;

    *ERX_DMACH_ON       = 0x0F;
    *ETX_DMACH_ON       = 0x0F;
    *CFG_ETH_DMA_MODE   = 1;    //  counter mode

    rx_cfg.deslen = eth_rx_descriptor_length;
    for ( i = 0; i < 4; i++ ) {
        rx_cfg.desba = ((unsigned int)priv->rx_desc[i] >> 2) & 0x0FFFFFFF;
        *ERX_DMA_CHANNEL_CONFIG(i) = rx_cfg;
    }

    tx_cfg.deslen = eth_tx_descriptor_length;
    for ( i = 0; i < 4; i++ ) {
        tx_cfg.desba = ((unsigned int)priv->tx_desc[i] >> 2) & 0x0FFFFFFF;
        *ETX_DMA_CHANNEL_CONFIG(i) = tx_cfg;
    }
}
...
static int __devinit ifx_atm_init(void)
{
    ...
    g_eth_net_dev = ifx_eth_fw_alloc_netdev(sizeof(struct eth_priv_data), "eth0", &eth_ops);
    if ( g_eth_net_dev == NULL )
        goto ALLOC_NETDEV_FAIL;
    priv = ifx_eth_fw_netdev_priv(g_eth_net_dev);

    ret = init_eth_priv_data();
    if ( ret != IFX_SUCCESS ) {
        err("INIT_ETH_PRIV_DATA_FAIL");
        goto INIT_ETH_PRIV_DATA_FAIL;
    }
    ...
    ret = ifx_eth_fw_register_netdev(g_eth_net_dev);
    if ( ret )
    ...
    ret = request_irq(ETH_MAILBOX_IGU0_INT, eth_irq_handler, IRQF_DISABLED, "eth_mailbox_isr", priv);
    if ( ret )
        goto REQUEST_IRQ_ETH_MAILBOX_IGU0_INT_FAIL;
    disable_irq(ETH_MAILBOX_IGU0_INT);
    ...
    IFX_REG_W32_MASK(0xFFFFFFFF, 0, MBOX_IGU0_IER);
    enable_irq(ETH_MAILBOX_IGU0_INT);
...
PP32_START_FAIL:
    free_irq(ETH_MAILBOX_IGU0_INT, priv);
REQUEST_IRQ_ETH_MAILBOX_IGU0_INT_FAIL:
    free_irq(PPE_MAILBOX_IGU1_INT, &g_atm_priv_data);
REQUEST_IRQ_PPE_MAILBOX_IGU1_INT_FAIL:
    ifx_eth_fw_unregister_netdev(g_eth_net_dev, 0);
REGISTER_NETDEV_FAIL:
ATM_DEV_REGISTER_FAIL:
    while ( port_num-- > 0 )
        atm_dev_deregister(g_atm_priv_data.port[port_num].dev);
INIT_ETH_PRIV_DATA_FAIL:
    clear_eth_priv_data();
    ifx_eth_fw_free_netdev(g_eth_net_dev, 0);
    g_eth_net_dev = NULL;
ALLOC_NETDEV_FAIL:
INIT_PRIV_DATA_FAIL:
    clear_priv_data();
...
static void __exit ifx_atm_exit(void)
{
    ...
    free_irq(ETH_MAILBOX_IGU0_INT, ifx_eth_fw_netdev_priv(g_eth_net_dev));
...
static int __init eth0addr_setup(char *line)
{
    ...
    p = g_my_ethaddr;
    memset(p, 0, sizeof(g_my_ethaddr));
    for ( i = 0; i < 6; i++ )
    {

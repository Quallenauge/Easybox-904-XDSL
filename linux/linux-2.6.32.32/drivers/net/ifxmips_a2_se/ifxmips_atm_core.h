...
#ifdef CONFIG_IFX_RED_MII_MAC_MODE
...
struct eth_priv_data {
    ...
    volatile struct rx_descriptor  *rx_desc[4];
    struct sk_buff                **rx_skb[4];
    unsigned int                    rx_desc_pos[4];

    volatile struct tx_descriptor  *tx_desc[4];
    struct sk_buff                **tx_skb[4];
    unsigned int                    tx_desc_pos[4];

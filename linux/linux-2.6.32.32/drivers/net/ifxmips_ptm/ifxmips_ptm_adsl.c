...
static struct net_device_stats *ptm_get_stats(struct net_device *dev)
{
    ...
    for ( ndev = 0; ndev < NUM_ENTITY(g_net_dev) && g_net_dev[ndev] != dev; ndev++ );
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32) && defined(CONFIG_IFX_PTM_RX_NAPI)
...
static int ptm_napi_poll(struct napi_struct *napi, int budget)
{
    ...
    for ( ndev = 0; ndev < NUM_ENTITY(g_net_dev) && g_net_dev[ndev] != napi->dev; ndev++ );
...
#if defined(CONFIG_IFX_PTM_RX_NAPI)
...
static irqreturn_t mailbox_irq_handler(int irq, void *dev_id)
{
  ...
  #if defined(CONFIG_IFX_PTM_RX_NAPI)
    #if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,32)
            netif_rx_schedule(g_net_dev[i]);
    #else
            napi_schedule(&g_ptm_priv_data.itf[i].napi);
    #endif
  #elif defined(CONFIG_IFX_PTM_RX_TASKLET)
            tasklet_schedule(&g_ptm_tasklet[i]);
  #endif
#endif
        }
    }
...
static void do_ptm_tasklet(unsigned long arg)
{
    ...
    ASSERT(arg >= 0 && arg < NUM_ENTITY(g_net_dev), "arg = %lu (wrong value)", arg);

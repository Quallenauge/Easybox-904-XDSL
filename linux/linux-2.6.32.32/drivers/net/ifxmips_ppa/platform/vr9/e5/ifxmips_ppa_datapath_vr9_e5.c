/******************************************************************************
**
** FILE NAME    : ifxmips_ppa_datapath_vr9_d5.c
** PROJECT      : UEIP
...
/*ctc*/
#ifdef CONFIG_ARC_PPA_PKT_CAPTURE
 #include "../pktcap.c"
#endif
...
#define WAN_RX_DESC_BASE                        ((volatile struct tx_descriptor *)SB_BUFFER(0x2600))                /*         64 each queue    */
#define DMA_TX_CH1_DESC_BASE                    (g_eth_wan_mode ? (g_eth_wan_mode == 3 /* ETH WAN */ && g_wanqos_en ? ((volatile struct rx_descriptor *)SB_BUFFER(0x2600)) : DMA_RX_CH1_DESC_BASE) : (volatile struct rx_descriptor *)WAN_RX_DESC_BASE)
 ...
 */
  ...
  struct flag_header {
    ...
    // 8 - 11h
    unsigned int    pkt_len             :16; //packet length
...
/*
 *  Internal Structure of Devices (ETH/ATM/PTM)
 */
...
static INLINE void ignore_space(char **, int *);
  ...
  static INLINE int print_mc(char *, int i, struct wan_rout_multicast_cmp_tbl *, struct wan_rout_multicast_act_tbl *);
#endif
 ...
 */
struct ppe_directpath_data g_ppe_directpath_data[4];

#if 1 // ARP add // bitonic added, ctc merged
...
static int arp_req_set(struct arpreq *r, struct net_device * dev)
{
	...
	if (dev == NULL) {
		struct flowi fl = { .nl_u = { .ip4_u = { .daddr = ip,
							 .tos = RTO_ONLINK } } };
		struct rtable * rt;
		if ((err = ip_route_output_key( NULL, &rt, &fl)) != 0)
			return err;
		dev = rt->u.dst.dev;
		ip_rt_put(rt);
		if (!dev)
...
void set_multicast_source_mac_address_to_arp_table(struct sk_buff *skb)
{
	// Get the source ip and mac address of multicast packets // bitonic
	struct iphdr	*iph;
	iph = (struct iphdr*)skb->data;
	if( (skb->pkt_type == PACKET_MULTICAST) && (iph->protocol==IPPROTO_IGMP) )
	{
		struct ethhdr *eth;
		...
		memset(&arp_req, 0x00, sizeof(struct arpreq));

		/* Inet sockaddr_in struct */
		memset(&addr, 0, sizeof(struct sockaddr_in));
		addr.sin_family = AF_INET;
		addr.sin_addr.s_addr = nIPv4; //inet_addr(ip);
		/* ARP arpreq struct */
		memcpy(&arp_req.arp_pa, &addr, sizeof(struct sockaddr));
		memcpy(arp_req.arp_ha.sa_data, mac, 6);
		//	req.arp_flags = ATF_PERM | ATF_COM;
		arp_req.arp_flags = ATF_COM;
		strcpy(arp_req.arp_dev, skb->dev->name);
		arp_req_set(&arp_req,NULL);
	}
}

#endif // end of ARP add // bitonic

// Wireless chip uses one of switch port to send and receive data from CPU.
// We can use this structure to find the physical switch port number.
typedef struct
{
...
/*
Process steps:
1. Wireless has two different channels. One is bridged as one lan interface and the other is isolated from lan interfaces.
2. Wireless driver uses add_isolation_mac() and remove_isolation_mac() to update isolation table.
3. One of switch port is assigned to receive and send data to wireless driver.
   => We must config two different VLANs(bridge: vlan 2; isolate: vlan 3) to this specified port(ex: port 5).
4. Switch driver uses query_isolation_mac() to find ont whether this packet is from isolated channel.
*/
#define SWITCH_PHYSICAL_PORT_TO_WIRELESS 5
#define WIRELESS_NORMAL_VLAN_ID 66
#define WIRELESS_ISOLATION_VLAN_ID 70

#define MAX_NUMBER_OF_MAC 64 // 32 * 2 both band

struct Ehdr_1Q {			/* Ethernet header with 802.1Q VLAN tag */
	unsigned char	to[6];
...
struct _iso_mac_tbl
{
 struct _iso_mac_entry iso_mac_entry[MAX_NUMBER_OF_MAC];
};

static struct _iso_mac_tbl  iso_mac_tbl={0,};
static struct semaphore     g_sem_iso;
static int                  g_sem_iso_inited=0;

#if 0
void remove_isolation_mac(unsigned char *mac)
{
	int i;

	if(g_sem_iso_inited==0){
	    g_sem_iso_inited=1;
	    sema_init(&g_sem_iso, 1);
	}

    down(&g_sem_iso);
	for (i=0;i<MAX_NUMBER_OF_MAC;i++)
		if (iso_mac_tbl.iso_mac_entry[i].valid)
			if (!memcmp(mac,iso_mac_tbl.iso_mac_entry[i].mac,6))
			{
				iso_mac_tbl.iso_mac_entry[i].valid = 0;
				printk("remove %02x:%02x:%02x:%02x:%02x:%02x from iso table\n",
...
void add_isolation_mac(unsigned char *mac)
{
	int i;
    ...
    remove_isolation_mac(mac);
    down(&g_sem_iso);
	for (i=0;i<MAX_NUMBER_OF_MAC;i++)
		if (iso_mac_tbl.iso_mac_entry[i].valid == 0)
		{
			iso_mac_tbl.iso_mac_entry[i].valid =1 ;
			memcpy(iso_mac_tbl.iso_mac_entry[i].mac,mac,6);
			printk("add %02x:%02x:%02x:%02x:%02x:%02x in iso table\n",
					mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
			break;
		}
	up(&g_sem_iso);
...
// TODO: Speed up query procedure
// Return 1 if this mac address is isolated
// else return 0
int query_isolation_mac(unsigned char *mac)
{
	int i, found=0;
	...
	for (i=0;i<MAX_NUMBER_OF_MAC;i++)
		if (iso_mac_tbl.iso_mac_entry[i].valid)
			...
			{
#if 0
				printk("query:%02x:%02x:%02x:%02x:%02x:%02x  in iso table\n",
					mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
#endif
				found = 1;
				break;
...
static void dump_cpu_egress_pkt_header_t(unsigned char *pData)
{
	int i;

	printk("[dump_cpu_egress_pkt_header_t] start\n");
	for(i=0; i<8; i++)
	{
		printk("0x%x ", *(pData+i));
	}
	printk("[dump_cpu_egress_pkt_header_t] end\n");
}

static void print_packet(unsigned char *pData, int len)
{
	int i;

	printk("\nLen=%d\n", len);
	for(i=0;i<len;i++){
		printk("%02x ",(u8)(pData[i]));
		if (i % 16 == 15)
			printk("\n");
	}
	printk("\n");
...
#if 0
EXPORT_SYMBOL(add_isolation_mac);
EXPORT_SYMBOL(remove_isolation_mac);
...
// end of stop flooding packet, bitonic
...
{
    ASSERT(((struct eth_priv_data *)ifx_eth_fw_netdev_priv(dev))->dev_id == IFX_PPA_PORT_PTM, "incorrect device");
...
static int ptm_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    ASSERT(priv->dev_id == IFX_PPA_PORT_PTM, "incorrect device");
    ...
    if ( f_full ) {
        ...
        IFX_REG_W32_MASK(0, CPU_TO_WAN_TX_SIG, MBOX_IGU1_ISRC);
    ...
    *(struct sk_buff **)((unsigned int)skb->data - byteoff - sizeof(struct sk_buff *)) = skb;
    ...
    reg_desc.dataptr = (unsigned int)skb->data & (0x1FFFFFFF ^ (DMA_ALIGNMENT - 1));
	...
	/*ctc*/
#ifdef CONFIG_ARC_PPA_PKT_CAPTURE
	ehdr=(struct Ehdr_1Q *)skb->data;
	if(ehdr->tpid==htons(0x8100))
	{
		tci=ntohs(ehdr->tci);
		vlan_id = tci & 0xFFF;
	...
	ebt_log_packet( skb, 0, 1, 0, 0, 0, vlan_id );
#endif
...
/* ctc */
#define	ARC_ETH_MAC	1
#ifdef ARC_ETH_MAC

unsigned char _ppa_eth0_mac_addr[6] = {0,0,0,0,0,0};

int eth_get_sys_mac_address(void *p)
{
	if ( p == 0 )
		return -1;

	if ( _ppa_eth0_mac_addr[0] == '\0' && _ppa_eth0_mac_addr[1] == '\0'
	  && _ppa_eth0_mac_addr[2] == '\0' && _ppa_eth0_mac_addr[3] == '\0'
	  && _ppa_eth0_mac_addr[4] == '\0' && _ppa_eth0_mac_addr[5] == '\0' )
	{
		return -2;
	}

    my_memcpy(p, _ppa_eth0_mac_addr, sizeof(_ppa_eth0_mac_addr));

	return 0;
}

EXPORT_SYMBOL(eth_get_sys_mac_address);
...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
  ...
  /*ctc*/
	...
	if(ehdr->tpid==htons(0x8100))
	{
		...
		if(vlan_id == WIRELESS_ISOLATION_VLAN_ID)
		{
			bToWireless = 1;
		}
	}
	...
	if(port == 0)
		ebt_log_packet( skb, 0, 1, (bToWireless == 1) ? 2 : 1, 0, 0, vlan_id );
	else
		ebt_log_packet( skb, 0, 1, 0, 0, 0, vlan_id );
#endif
...
static int eth_set_mac_address(struct net_device *dev, void *p)
{
  ...
  /*ctc*/
	...
	if ( strcmp(dev->name,"eth0") == 0)
	    my_memcpy(_ppa_eth0_mac_addr, addr->sa_data, sizeof(_ppa_eth0_mac_addr));
  #endif
    ...
    case ETH_MAP_PKT_PRIO_TO_Q:
        {
            ...
            if ( cmd.qid < 0 )
                return -EINVAL;
            if ( cmd.qid >= ((g_wan_itf & (1 << port)) && g_wanqos_en && (g_eth_wan_mode == 3) ? g_wanqos_en : 4) )
                return -EINVAL;
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    skb->len  = len;
    dump_skb(skb, DUMP_SKB_LEN, "dma_rx_int_handler raw data", 0, dma_dev->current_rx_chan, 0, 0);
	...
	// bitonic add
    //dump_cpu_egress_pkt_header_t(skb->data-8);
...
/* Terry 20131108, To pass rtl switch port information in MCAST packets */
    ...
    if ( header->acc_done && header->dest_list )        //  acc_done == 1 && dest_list != 0
    {
        ...
        if ( (header->dest_list & (1 << 0)) )           //  0 - eth0
        {
            eth_xmit(skb, 0, 2, 2, header->qid);
            return 0;
        }
        else if ( (header->dest_list & (1 << 1)) )      //  1 - eth1
        {
            eth_xmit(skb, 1, 2, 2, header->qid);
            return 0;
        }
        else if ( (header->dest_list & (1 << 7)) )      //  7 - ptm0
        {
            ptm_hard_start_xmit(skb, g_ptm_net_dev[0]);
            return 0;
        }
        else
        {
    ...
    off = header->src_itf;
	/*ctc*/
#if (defined(CONFIG_ARC_PPA_FILTER) || defined(CONFIG_ARC_PPA_PKT_CAPTURE))
	ehdr=(struct Ehdr_1Q *)skb->data;
	if(ehdr->tpid==htons(0x8100))
	{
		tci=ntohs(ehdr->tci);
	...
	pPtr = (uint32_t*)(skb->cb+sizeof(skb->cb)) - 1;
	pPtr = (uint32_t*)((uint32_t)pPtr & ~0x3);

	if(bVlanPacket)
	{  //with vlan tag
		*pPtr = (uint32_t)(skb->data + 4);
	}
	else
	{
		*pPtr = (uint32_t)skb->data;
	}
#endif
    ...
    case 1: //  MII1
        ...
        if ( netif_running(g_eth_net_dev[off]) )
        {
            ...
            if(source_port == SWITCH_PHYSICAL_PORT_TO_WIRELESS)
			{
	            ehdr=(struct Ehdr_1Q *)skb->data;
	            if(ehdr->tpid==htons(0x8100))
	            {  //with vlan tag
...
//               		print_packet(skb->data, 32);
			...
			}
            {
				...
				if(IsFloodingPakcet(skb->data, skb->len))
				{
				    dev_kfree_skb_any(skb);
				    return 0;
				}
...
#ifdef CONFIG_ARC_PPA_PKT_CAPTURE
				if(off==0)
					ebt_log_packet( skb, 0, 0, (bFromWireless == 1) ? 2 : 1, 0, 0, vlan_id );
				else
...
static INLINE void init_pmu(void)
{
    ...
    PPE_TPE_PMU_SETUP(IFX_PMU_ENABLE);  //  AHB Slave
...
static INLINE void start_cpu_port(void)
{
  #if 0 /* ctc disable all ports by default */
    int i;
  #endif
...
static INLINE int pp32_start(int pp32, int is_e1)
{
    ...
    /*  download firmware   */
    if ( is_e1 )
        ret = pp32_download_code(pp32, e1_firmware_binary_code, sizeof(e1_firmware_binary_code) / sizeof(*e1_firmware_binary_code), e1_firmware_binary_data, sizeof(e1_firmware_binary_data) / sizeof(*e1_firmware_binary_data));
    else
...
static INLINE int init_local_variables(void)
{
    ...
    printk("[init_local_variables] g_wanqos_en=%d\n", g_wanqos_en);
    ...
    for ( i = 0; i < 2; i++ )
    {
        ...
        tx_num_q = (g_wan_itf & (1 << i)) && (g_eth_wan_mode == 3 /* ETH WAN */ && g_wanqos_en) ? __ETH_WAN_TX_QUEUE_NUM : 4;
        q_step = tx_num_q - 1;
        ...
        if ( (g_wan_itf & (1 << i)) && (g_eth_wan_mode == 3 /* ETH WAN */ && g_wanqos_en) )
...
static INLINE void init_ptm_tables(void)
{
    ...
    cfg_std_data_len.byte_off = 0;  //  this field replaces byte_off in rx descriptor of VDSL ingress
...
static INLINE int alloc_dma(void)
{
    ...
    if ( g_eth_wan_mode == 0 || g_wanqos_en )
    {
        ...
        {
            ASSERT(((u32)(*pskb)->data & (DMA_ALIGNMENT - 1)) == 0, "CPU to WAN TX data pointer (0x%08x) must be 8 DWORDS aligned", (u32)(*pskb)->data);
            *p++ = 0x30000000;
        ...
        {
            ASSERT(((u32)(*pskb)->data & (DMA_ALIGNMENT - 1)) == 0, "WAN (PTM/ETH) QoS TX data pointer (0x%08x) must be 8 DWORDS aligned", (u32)(*pskb)->data);
            *p++ = 0x30000000;
            *p++ = (u32)(*pskb++)->data & 0x1FFFFFE0;
        }
...
static int print_fw_ver(char *buf, int buf_len)
{
    ...
    for ( i = 0; i < NUM_ENTITY(g_fw_ver); i++ )
    {
        if ( *(unsigned int *)(&g_fw_ver[i]) == 0 )
            continue;

        len += snprintf(buf + len, buf_len - len, "PPE %d firmware info:\n", i);
        len += snprintf(buf + len, buf_len - len, "  Version ID: %d.%d.%d.%d.%d.%d\n", (int)g_fw_ver[i].family, (int)g_fw_ver[i].fwtype, (int)g_fw_ver[i].interface, (int)g_fw_ver[i].fwmode, (int)g_fw_ver[i].major, (int)g_fw_ver[i].minor);
        if ( g_fw_ver[i].family > NUM_ENTITY(fw_ver_family_str) )
            len += snprintf(buf + len, buf_len - len, "  Family    : reserved - %d\n", (int)g_fw_ver[i].family);
        else
            len += snprintf(buf + len, buf_len - len, "  Family    : %s\n", fw_ver_family_str[g_fw_ver[i].family]);
        if ( g_fw_ver[i].fwtype > NUM_ENTITY(fw_ver_type_str) )
            len += snprintf(buf + len, buf_len - len, "  FW Type   : reserved - %d\n", (int)g_fw_ver[i].fwtype);
        else
            len += snprintf(buf + len, buf_len - len, "  FW Type   : %s\n", fw_ver_type_str[g_fw_ver[i].fwtype]);
        if ( g_fw_ver[i].interface > NUM_ENTITY(fw_ver_interface_str) )
            len += snprintf(buf + len, buf_len - len, "  Interface : reserved - %d\n", (int)g_fw_ver[i].interface);
        else
            len += snprintf(buf + len, buf_len - len, "  Interface : %s\n", fw_ver_interface_str[g_fw_ver[i].interface]);
        if ( g_fw_ver[i].fwmode > NUM_ENTITY(fw_ver_mode_str) )
            len += snprintf(buf + len, buf_len - len, "  Mode      : reserved - %d\n", (int)g_fw_ver[i].fwmode);
        else
            len += snprintf(buf + len, buf_len - len, "  Mode      : %s\n", fw_ver_mode_str[g_fw_ver[i].fwmode]);
        len += snprintf(buf + len, buf_len - len, "  Release   : %d.%d\n", (int)g_fw_ver[i].major, (int)g_fw_ver[i].minor);
    }
...
static INLINE void proc_file_create(void)
{
    ...
    if ( g_ipv6_acc_en )
    {
        create_proc_read_entry("ipv6_ip",
                            0,
                            g_eth_proc_dir,
                            proc_read_ipv6_ip,
                            NULL);
    ...
    res = create_proc_read_entry("tfwdbg",
                                   0,
                                   g_eth_proc_dir,
                                   NULL,
                                   NULL);
    if(res)
        res->write_proc = proc_write_fwdbg;
    ...
    res = create_proc_entry("direct_forwarding",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_directforwarding;
        res->write_proc = proc_write_directforwarding;
    }

    res = create_proc_entry("clk",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_clk;
        res->write_proc = proc_write_clk;
    }

#if defined(ENABLE_NAPI) && ENABLE_NAPI
    res = create_proc_entry("napi",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_napi;
	...
	// bitonic
	/*
	If there are many incoming fragment packets, IAD will take more CPU time to process these packets.
	Our idea is to drop fragment packets within one reasonable interval when user space check task set command to driver.
	*/
    res = create_proc_entry("stop_flooding_packet",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_stop_flooding_packet;
        res->write_proc = proc_write_stop_flooding_packet;
    }
	//
}
...
{
	// bitonic
    remove_proc_entry("stop_flooding_packet",
                      g_eth_proc_dir);
...
static int proc_read_mib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    int showptm = 0;
    struct eth_priv_data *priv0, *priv1, *priv2 = NULL;
    char *itf_name[] = {"eth0", "eth1", "cpu", "ext_int1", "ext_int2", "ext_int3", "ext_int4", "ptm0"};
    char *row_name[] = {"ig_fast_brg_pkts", "ig_fast_brg_bytes", "ig_fast_rt_ipv4_udp_pkts", "ig_fast_rt_ipv4_tcp_pkts", "ig_fast_rt_ipv4_mc_pkts", "ig_fast_rt_ipv4_bytes", "ig_fast_rt_ipv6_udp_pkts", "ig_fast_rt_ipv6_tcp_pkts", NULL, "ig_fast_rt_ipv6_bytes", NULL, "ig_cpu_pkts", "ig_cpu_bytes", "ig_drop_pkts", "ig_drop_bytes", "eg_fast_pkts"};
    int i, j, k, h;
    ...
    if ( g_ptm_net_dev[0] )
    {
        ...
        priv2 = ifx_eth_fw_netdev_priv(g_ptm_net_dev[0]);
        showptm = 1;
    }
    ...
    len += sprintf(page + len, "  Driver  (eth0, eth1, ptm0)\n");
    len += sprintf(page + len, "    rx_packets: %10u, %10u, %10u\n",  priv0->rx_packets, priv1->rx_packets, showptm ? priv2->rx_packets : 0);
    len += sprintf(page + len, "    rx_bytes:   %10u, %10u, %10u\n",  priv0->rx_bytes,   priv1->rx_bytes,   showptm ? priv2->rx_bytes : 0);
    len += sprintf(page + len, "    rx_dropped: %10u, %10u, %10u\n",  priv0->rx_dropped, priv1->rx_dropped, showptm ? priv2->rx_dropped : 0);
    len += sprintf(page + len, "    tx_packets: %10u, %10u, %10u\n",  priv0->tx_packets, priv1->tx_packets, showptm ? priv2->tx_packets : 0);
    len += sprintf(page + len, "    tx_bytes:   %10u, %10u, %10u\n",  priv0->tx_bytes,   priv1->tx_bytes,   showptm ? priv2->tx_bytes : 0);
    len += sprintf(page + len, "    tx_errors:  %10u, %10u, %10u\n",  priv0->tx_errors,  priv1->tx_errors,  showptm ? priv2->tx_errors : 0);
    len += sprintf(page + len, "    tx_dropped: %10u, %10u, %10u\n",  priv0->tx_dropped, priv1->tx_dropped, showptm ? priv2->tx_dropped : 0);
...
static int proc_read_wanmib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + len, "RX:\n");
    len += sprintf(page + len, "  wrx_total_pdu:   ");
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(wrx_total_pdu[i]));
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_RX_MIB_TABLE(i)->wrx_dropdes_pdu);
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_RX_MIB_TABLE(i)->wrx_total_bytes);
    }
    len += sprintf(page + len, "\n");

    len += sprintf(page + len, "TX:\n");
    len += sprintf(page + len, "  wrx_total_pdu:           ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_total_pdu);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wrx_total_bytes:         ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->small_pkt_drop_cnt);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_total_pdu:           ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->total_pkt_drop_cnt);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_total_bytes:         ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_total_bytes);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_cpu_drop_small_pdu:  ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_cpu_drop_small_pdu);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_cpu_drop_pdu:        ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_cpu_drop_pdu);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_fast_drop_small_pdu: ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_fast_drop_small_pdu);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wtx_fast_drop_pdu:       ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wtx_fast_drop_pdu);
    }
    len += sprintf(page + len, "\n");
...
static void fwdbg_clear_ethbuf(char **tokens, int token_num)
{
    ...
    ppa_memset((void *)cbase, 0, pg_num * pg_size * sizeof(uint32_t));
...
static int proc_read_clk(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    if ( valid )
        len += sprintf(page + len, "PPE clock - %uM\n", clk);
    else
        len += sprintf(page + len, "PPE clock - %uM (unknown option %u)\n", clk, (*VR9_CGU_CLKFSR >> 16) & 0x07);
...
static int proc_read_prio(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    if ( g_eth_wan_mode == 0 )
    {
        len += sprintf(page + len,    "  ptm0     :");
        for ( j = 0; j < NUM_ENTITY(g_ptm_prio_queue_map); j++ )
            len += sprintf(page + len,"  %d", g_ptm_prio_queue_map[j]);
        len += sprintf(page + len,    "\n");
    }
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        else if ( port != ~0 )
        {
            if ( stricmp(p1, "p") == 0 || stricmp(p1, "prio") == 0 )
            {
                ...
                if ( port >= 0 && port <= 1 )
                {
                    if ( prio >= 0 && prio < NUM_ENTITY(g_eth_prio_queue_map[port]) )
                    {
                        if ( queue >= 0 )
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                if ( port >= 0 && port <= 1 )
                {
                    if ( queue >= 0 && ((port == 0 && queue <= 3) || (port == 1 && queue < (g_eth_wan_mode == 3 && g_wanqos_en ? g_wanqos_en : 4))) )
                    {
                        if ( prio >= 0 )
                            g_eth_prio_queue_map[port][prio] = queue;
                    }
                    ...
                    {
                        err("queue (%d) is out of range 0 - %d", queue, port == 0 ? 3 : (g_eth_wan_mode == 3 && g_wanqos_en ? g_wanqos_en - 1 : 3));
                    }
                }
                else if ( port == 2 )
                {
...
static INLINE void dump_skb(struct sk_buff *skb, u32 len, char *title, int port, int ch, int is_tx, int enforce)
{
    ...
    if ( !enforce && !(g_dbg_enable & (is_tx ? DBG_ENABLE_MASK_DUMP_SKB_TX : DBG_ENABLE_MASK_DUMP_SKB_RX)) )
        return;
    ...
    case PPA_GENERIC_DATAPATH_GET_PPE_VERION:
        {
            ...
            version->type   = g_fw_ver[version->index].fwtype;
            version->itf    = g_fw_ver[version->index].interface;
            version->mode   = g_fw_ver[version->index].fwmode;
...
{
    struct eth_priv_data *priv = ifx_eth_fw_netdev_priv(arg);
    ...
    else
    {
        ...
        else if ( g_eth_wan_mode == 0 )
        {
            if ( priv->dev_id == IFX_PPA_PORT_PTM )
                return g_ptm_prio_queue_map[skb->priority > 7 ? 7 : skb->priority];
        }
    }
...
static int __init ppe_eth_init(void)
{
    ...
    printk("Loading E5 (MII0/1) driver, wanqos_en=%d ...... \n", wanqos_en);

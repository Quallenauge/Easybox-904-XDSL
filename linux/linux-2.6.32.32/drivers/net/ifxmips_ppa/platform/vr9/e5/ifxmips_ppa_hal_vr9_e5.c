/******************************************************************************
**
** FILE NAME    : ifxmips_ppa_hal_vr9_d5.c
** PROJECT      : UEIP
...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        else if ( wan_itf & IFX_PPA_DEST_LIST_ATM ) qos_queue_portid = 7;
        else
        {
            qos_queue_portid = -1; //invalid
            res = IFX_FAILURE;
...
int proc_read_qos(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    ...
    {
        llen = sprintf(pstr, "\n  Note: PPA QOS is disabled and wan_itf=%x  mixed_itf=%x\n", *CFG_WAN_PORTMAP, *CFG_MIXED_PORTMAP);
        pstr += llen;
        len += llen;
    }
...
/*
    uint32_t flags[] = {
        ...
        IFX_PPA_PHYS_PORT_FLAGS_MODE_EXT_LAN_VALID | IFX_PPA_PHYS_PORT_FLAGS_EXT_CPU0,
        GEN_MODE_CFG1->sys_cfg == 0 ? IFX_PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID : 0
    };
*/
...
ADD_ROUTING_ENTRY_GOON:
    ...
    dbg("%s, hash = %d, entry = %d, pcomare = 0x%08x, paction = 0x%08x\n", f_is_lan ? "LAN" : "WAN", hash, entry, (unsigned int)pcompare, (unsigned int)paction);
...
int32_t set_ctrl_qos_rate( uint32_t portid, uint32_t enable, uint32_t flag)
{
    ...
    // if ( !tx_qos_cfg.eth1_qss ) return IFX_FAILURE;  //Note, For E5 ptm wan mode, the eth1_qss is disabled.
...
int32_t set_qos_rate( uint32_t portid, uint32_t queueid, uint32_t rate, uint32_t burst, uint32_t flag)
{
    ...
    {
        if( qos_queue_portid == 7)//PTM mode and there is no ported based rate shaping since DSL has its link rate
            return IFX_FAILURE;

        //QOS Note: otherwise it is E5 ethernet wan mode, so regard it as port based rate shaping, not queue based rate shaping
        qos_cfg = *WTX_EG_Q_PORT_SHAPING_CFG(0);
   ...
   case PPA_GENERIC_HAL_GET_PPE_FW_VERSION:
        {
            ...
            return ifx_ppa_drv_datapath_generic_hook(PPA_GENERIC_DATAPATH_GET_PPE_VERION, buffer, 0 );
        }

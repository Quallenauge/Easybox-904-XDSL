...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
    ...
    dump_skb(skb, DUMP_SKB_LEN, "ppe_send", 7, reg_desc.qid, 1);
	/*ctc*/
#ifdef CONFIG_ARC_PPA_PKT_CAPTURE
	ehdr=(struct Ehdr_1Q *)(skb->data + 10); //  ignore AAL5 header
	...
	ebt_log_packet( skb, 10, 1, 0, vcc->vpi, vcc->vci, vlan_id );
#endif
...
static int ppe_send_oam(struct atm_vcc *vcc, void *cell, int flags)
{
    ...
    dump_skb(skb, DUMP_SKB_LEN, "ppe_send_oam", 7, reg_desc.qid, 1);
...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
  ...
  /*ctc*/
	...
	// void ebt_log_packet( const struct sk_buff *skb, int off, int dir, int nInterfaceType, short vpi, int vci, unsigned short vlan_id);
	if(port == 0)
		ebt_log_packet( skb, 0, 1, (bToWireless == 1) ? 2 : 1, 0, 0, vlan_id ); // lan or wlan
	else
		ebt_log_packet( skb, 0, 1, 0, 0, 0, vlan_id ); // wan
#endif
    ...
    if ( (mailbox_signal & WAN_RX_SIG(1)) ) //  OAM
    {
        ...
        do
        {
            while ( 1 )
            {
                ...
                skb->len = CELL_SIZE;
                dump_skb(skb, DUMP_SKB_LEN, "mailbox_irq_handler - OAM", 7, 0, 0);
#endif
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    dump_skb(skb, DUMP_SKB_LEN, "dma_rx_int_handler raw data", header->src_itf, dma_dev->current_rx_chan, 0);
...
/* Terry 20131108, To pass rtl switch port information in MCAST packets */
    ...
    if ( header->acc_done && header->dest_list )        //  acc_done == 1 && dest_list != 0
    {
        ...
        else if ( (header->dest_list & (1 << 7)) )      //  7 - DSL, 6 - res
        {
            ...
            *(struct sk_buff **)((u32)skb->data - byteoff - sizeof(struct sk_buff *)) = skb;
            ...
            dump_skb(skb, DUMP_SKB_LEN, "ppe_send", 7, 0, 1);
	...
	/*ctc*/
        ...
        if ( netif_running(g_eth_net_dev[off]) )
        {
            ...
            // check this packet is from wireless.
        ...
        if ( (vcc = g_atm_priv_data.connection[conn].vcc) != NULL )
        {
            if ( atm_charge(vcc, skb->truesize) )
            {
                ...
                //else
                {
                    ...
                    dump_skb(skb, DUMP_SKB_LEN, "dma_rx_int_handler AAL5 encapsulated", off, dma_dev->current_rx_chan, 0);
					...
					if(bVlanPacket)
					{  //with vlan tag
						*pPtr = (uint32_t)(skb->data + 4 + 10);
					}
					else
					{
						*pPtr = (uint32_t)(skb->data + 10);
					}
#endif

					if(IsFloodingPakcet(skb->data + 10, skb->len - 10))
					{
					    dev_kfree_skb_any(skb);
					    return 0;
					}
					...
					#ifdef CONFIG_ARC_PPA_PKT_CAPTURE
				  	// void ebt_log_packet( const struct sk_buff *skb, int off, int dir, int nInterfaceType, short vpi, int vci, unsigned short vlan_id);
					ebt_log_packet( skb, 10, 0, 0, vcc->vpi, vcc->vci, vlan_id );
					#endif
                }
...
static INLINE int pp32_download_code(int pp32, const u32 *code_src, unsigned int code_dword_len, const u32 *data_src, unsigned int data_dword_len)
{
    ...
    if ( pp32 == 0 && code_dword_len > CDM_CODE_MEMORYn_DWLEN(0) + CDM_CODE_MEMORYn_DWLEN(1) )
    {
        clr = 0xCF;
        set = 2 << 6;
    }
...
static INLINE int alloc_dma(void)
{
    ...
    //g_dma_device->max_rx_chan_num = 6;
...
static int proc_read_mib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + len, "  tx_bytes:   %10u, %10u,        N/A\n", priv0->tx_bytes,    priv1->tx_bytes);
    ...
    len += sprintf(page + len, "  wrx_drophtu_cell: %10u\n", DSL_WAN_MIB_TABLE->wrx_drophtu_cell);
    len += sprintf(page + len, "  wrx_dropdes_pdu:  %10u\n", DSL_WAN_MIB_TABLE->wrx_dropdes_pdu);
    len += sprintf(page + len, "  wrx_correct_pdu:  %10u\n", DSL_WAN_MIB_TABLE->wrx_correct_pdu);
    len += sprintf(page + len, "  wrx_err_pdu:      %10u\n", DSL_WAN_MIB_TABLE->wrx_err_pdu);
    len += sprintf(page + len, "  wrx_dropdes_cell: %10u\n", DSL_WAN_MIB_TABLE->wrx_dropdes_cell);
    len += sprintf(page + len, "  wrx_correct_cell: %10u\n", DSL_WAN_MIB_TABLE->wrx_correct_cell);
    len += sprintf(page + len, "  wrx_err_cell:     %10u\n", DSL_WAN_MIB_TABLE->wrx_err_cell);
    len += sprintf(page + len, "  wrx_total_byte:   %10u\n", DSL_WAN_MIB_TABLE->wrx_total_byte);
    len += sprintf(page + len, "  wtx_total_pdu:    %10u\n", DSL_WAN_MIB_TABLE->wtx_total_pdu);
    len += sprintf(page + len, "  wtx_total_cell:   %10u\n", DSL_WAN_MIB_TABLE->wtx_total_cell);
    len += sprintf(page + len, "  wtx_total_byte:   %10u\n", DSL_WAN_MIB_TABLE->wtx_total_byte);

#if 0  //proc stack overflow so remove it
    if ( g_eth_wan_mode == 0 )
    {
        len += sprintf(page + len, "DSL Queue DROP MIB\n");
        for ( i = 0; i < ATM_QUEUE_NUMBER; i++ )
        {
            len += sprintf(page + len, "  queue%02d: %10u", i, DSL_QUEUE_TX_DROP_MIB_TABLE(i)->pdu);
            if ( (i % 4) == 3 )
                len += sprintf(page + len, "\n");
        }
        len += sprintf(page + len, "\n");
...
static int proc_write_mib(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    else if ( (mib_itf & 0x80000000) )
    {
        ...
        if ( (mib_itf & 0xC0) )
        {
            volatile struct dsl_queue_drop_mib *dsl_drop_mib;
...
//                memset((void *)DSL_WAN_VC_MIB_TABLE(i), 0, sizeof(struct dsl_wan_vc_mib_table));
            ...
            for ( i = 0; i < ATM_QUEUE_NUMBER; i++ )
            {
                ...
                if ( g_eth_wan_mode == 0 )
                {
                    dsl_drop_mib = DSL_QUEUE_TX_DROP_MIB_TABLE(i);
                    memset((void *)dsl_drop_mib, 0, sizeof(DSL_QUEUE_TX_DROP_MIB_TABLE(i)));
                }
            }
        }
        for ( i = 0; i < 8; i++ )
        ...
        else if ( port != ~0 || conn >= 0 )
        {
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                if ( port >= 0 && port <= 1 )
                {
                    ...
                    {
                        err("queue (%d) is out of range 0 - %d", queue, (g_wan_itf & (1 << port)) && g_wanqos_en ? g_wanqos_en - 1 : 3);
                    }
                }
                else if ( conn >= 0 )
                {

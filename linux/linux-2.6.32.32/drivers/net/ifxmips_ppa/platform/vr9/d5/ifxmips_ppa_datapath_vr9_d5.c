...
#include "ifxmips_ppa_hal_vr9_d5.h"
...
#define DMA_TX_CH1_DESC_BASE                    (g_wanqos_en ? ((volatile struct rx_descriptor *)SB_BUFFER(0x2600)) : DMA_RX_CH1_DESC_BASE)
...
#define WIRELESS_ISOLATION_VLAN_ID 71
    ...
    case ETH_MAP_PKT_PRIO_TO_Q:
        {
            ...
            if ( cmd.qid < 0 )
                return -EINVAL;
            if ( cmd.qid >= ((g_wan_itf & (1 << port)) && g_wanqos_en ? g_wanqos_en : 4) )
                return -EINVAL;
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        else if ( port != ~0 )
        {
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                {
                    err("queue (%d) is out of range 0 - %d", queue, port == 0 ? 3 : (g_wanqos_en ? g_wanqos_en - 1 : 3));
                }
            }
            else
            {

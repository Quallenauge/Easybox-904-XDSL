/******************************************************************************
**
** FILE NAME    : ifxmips_ppa_hal_amazon_se_e4.c
** PROJECT      : UEIP
...
** HISTORY
** $Date        $Author         $Comment
** 22 OCT 2010  Xu Liang        Initiate Version
*******************************************************************************/
...
#define MAX_ROUTING_ENTRIES                     88  //  routing+bridging mode 88, pure routing mode 128
...
struct dsl_wan_mib_table {
    ...
    unsigned int    wrx_dropdes_pdu;    //  1
    unsigned int    wrx_err_pdu;        //  2
    unsigned int    res1;               //  3
    unsigned int    res2;               //  4
    unsigned int    res3;               //  5
    unsigned int    wrx_idle_bytes;     //  6
    unsigned int    res4;               //  7
    unsigned int    wrx_nonidle_cw;     //  8
    unsigned int    wrx_idle_cw;        //  9
    unsigned int    wrx_err_cw;         //  A
    unsigned int    res5;               //  B
...
{
    return 4;   //  support ETH0, PTM0, CPU, and 1 USB/WLAN port.
}
                        ...
                        PPA_IFNAME ifname[PPA_IF_NAME_SIZE])
{
    ...
    uint32_t flags[] = {
        ...
        ETH_PORTS_CFG->eth0_type == 0 ? IFX_PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID : 0,
                     ...
                     uint32_t vlan_port_map)
{
    ...
    case IFX_PPA_BRG_VLAN_IG_COND_TYPE_DEF:
        ...
        else
        {
            ...
            DEFAULT_ITF_VLAN_MAP(ig_criteria)->outer_vlan_ix = outer_vlan_ix; //sgh add
            DEFAULT_ITF_VLAN_MAP(ig_criteria)->dest_qos = dest_qos;  //sgh add
            ppa_lock_release(&g_itf_cfg_lock);
            return 0;
        }
    case IFX_PPA_BRG_VLAN_IG_COND_TYPE_SRC_IP:
...
void del_all_vlan_map(void)
{
    ...
    volatile struct vlan_vlan_map *p_vlan_table; //sgh add
    ...
    p_vlan_table = (volatile struct vlan_vlan_map *)VLAN_VLAN_MAP(0); //sgh
...
ADD_BRIDGING_ENTRY_GOON:
    ...
    *BRG_FWD_HIT_STAT_TBL(entry >> 5) &= ~(1 << (BITSIZEOF_UINT32 - 1 - (entry & 0x1F)));
    ...
    ((uint32_t *)BRIDGING_FORWARD_TBL(entry))[2] = ((uint32_t *)&action)[2];
...
{   /*shaoguoh: it seems only support 4 ports, not 8 ports, otherwise clearmib will cause problem */

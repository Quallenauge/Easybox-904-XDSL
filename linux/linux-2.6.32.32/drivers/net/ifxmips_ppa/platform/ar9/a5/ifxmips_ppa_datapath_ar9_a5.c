...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    pkth.qid    = 0;    //  0 is lowest priority, 3 is highest priority
...
static INLINE int eth_xmit(struct sk_buff *skb, unsigned int port, int ch, int spid, int prio)
{
    struct sw_eg_pkt_header pkth = {spid:   spid,   //  CPU/app0/app1/app2
                                    ...
                                    qid:    prio,   //  0 is lowest priority, 3 is highest priority
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    dbg("The payload offset: %d:(0x%x)",  sizeof(struct flag_header) + off, sizeof(struct flag_header) + off);
...
static INLINE void proc_file_create(void)
{
    ...
    res = create_proc_entry("portmib",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = physical_port_counters_proc_read;
        res->write_proc = physical_port_counters_proc_write;
    }
    ...
    if ( res )
    {
        ...
        res->write_proc = port0_counters_proc_write;
    }
    ...
    if ( res )
    {
        ...
        res->write_proc = port1_counters_proc_write;
    }
    ...
    if ( res )
    {
        ...
        res->write_proc = port2_counters_proc_write;
    }

    res = create_proc_entry("mactable",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_mactable;
    ...
    res = create_proc_entry("flowcontrol",
                            0,
                            g_eth_proc_dir);
    if ( res )
    {
        res->read_proc  = proc_read_flowcontrol;

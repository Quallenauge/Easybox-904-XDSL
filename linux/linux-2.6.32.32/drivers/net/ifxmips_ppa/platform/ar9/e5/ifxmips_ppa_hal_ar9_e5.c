/******************************************************************************
**
** FILE NAME    : ifxmips_ppa_hal_ar9_e5.c
** PROJECT      : UEIP
...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        else if ( wan_itf & IFX_PPA_DEST_LIST_PTM ) qos_queue_portid = 7;
        else
        {
            qos_queue_portid = -1; //invalid
            res = IFX_FAILURE;
...
/*!
  ...
  \param p_if       get interface supported by firmware
  \param p_mode     get firmware working mode
                           ...
                           uint32_t *p_entry)
{
    ...
    *AMAZON_S_SW_ADR_TB_CTL1 = p_item->mac1 | ((dest_list & 0xFF) << 20);
...
//    if( (g_wan_itf & IFX_PPA_DEST_LIST_PTM) /*DSL WAN mode */ && (p_vlan->rx_if_id == 6 || p_vlan->rx_if_id == 7) )
//    {

/******************************************************************************
**
** FILE NAME    : ifxmips_ppa_proc_ar9_e5.c
** PROJECT      : UEIP
** MODULES      : PTM + MII0/1 Acceleration Package (PPA)
**
** DATE         : 20 DEC 2010
** AUTHOR       : Xu Liang
...
/*!
  \file ifxmips_ppa_proc_ar9_e5.c
  \ingroup AMAZON_S_PPA_PPE_E5_HAL
...
int ifx_ppa_drv_proc_write_hal_mib(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    for (i = 0; i < get_phys_port_num(); i ++ )
    {
        ...
        {
            pmib = ITF_MIB_TBL(i);
            ppa_memset((void *)pmib, 0, sizeof(struct itf_mib));
        }
    }
    ...
    if ( eth_clear == 0xFF )
    {
        volatile struct wtx_qos_q_mib_table *wan_mib;
        for ( i = 0; i < PPE_MAX_ETH1_QOS_QUEUE; i++ )
        {
            wan_mib = WTX_QOS_Q_MIB_TABLE(i);
            ppa_memset((void *)wan_mib, 0, sizeof(struct wtx_qos_q_mib_table));
        }
    }
#endif

...
#include "ifx_ppa_stack_al.h"
#include "ifx_ppa_api_mfe.h"
#include "ifx_ppe_drv_wrapper.h"

#ifdef CONFIG_IFX_PPA_MFE

int32_t ifx_ppa_multifield_control(uint8_t enable, uint32_t flag)  //hook
{
    PPE_ENABLE_CFG mfe_cfg={0};
    mfe_cfg.f_enable = enable;
    mfe_cfg.flags = flag;
    
    return ifx_ppa_drv_multifield_control( &mfe_cfg, 0);
}

int32_t ifx_ppa_get_multifield_status(uint8_t *enable, uint32_t flag)  //hook
{
    int32_t res;

    PPE_ENABLE_CFG mfe_cfg={0};

    res = ifx_ppa_drv_get_multifield_status( &mfe_cfg, 0);
    if( enable)
    {    
        if( res == IFX_SUCCESS )
...
int32_t ifx_ppa_get_multifield_max_flow(uint32_t flag)   //hook
{
    int32_t res;
    ...
    res = ifx_ppa_drv_get_multifield_max_entry( &count, 0);

    if( res ) return count.num;
    else return 0;
}
...
static int32_t ifx_ppa_auto_learn_multifield_flow( PPA_MULTIFIELD_FLOW_INFO *p_multifield_info) //internal
{ //note, in vlan information auto-learnign, here set all vlan related mask to zero
    ...
    if ( p_multifield_info->cfg0.vlan_info.bfauto )
    {
        ...
        if( ppa_netif_update(NULL, p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname)  != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: ppa_netif_update fail for rx interface %s\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            return IFX_FAILURE;
        }

         if( ppa_netif_update(NULL, p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname)  != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: ppa_netif_update fail for tx interface %s\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
            return IFX_FAILURE;
        }

        if ( ppa_netif_lookup(p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname, &rx_ifinfo) != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: Rx interface %s ppa_netif_lookup fail\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            return IFX_FAILURE;
        }
        if ( ppa_netif_lookup(p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname, &tx_ifinfo) != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: Tx interface %s ppa_netif_lookup fail\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }
        if ( !(rx_ifinfo->flags & NETIF_PHYS_PORT_GOT) )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: Rx interface %s no physical port id\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }
        if ( !(tx_ifinfo->flags & NETIF_PHYS_PORT_GOT) )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: Tx interface %s no physical port id\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
            res = IFX_FAILURE;
            goto EXIT;
        }

        ppa_memset( &p_multifield_info->cfg0.vlan_info.vlan_info_manual, 0, sizeof(p_multifield_info->cfg0.vlan_info.vlan_info_manual) );
        p_multifield_info->cfg0.vlan_info.vlan_info_manual.rx_if_id = rx_ifinfo->phys_port;
        p_multifield_info->cfg0.vlan_info.vlan_info_manual.tx_if_id = tx_ifinfo->phys_port;

        if( rx_ifinfo->flags & NETIF_VLAN_CANT_SUPPORT )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: rx interface %s cannot support too many vlan tag\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.rx_ifname);
        }
        if( tx_ifinfo->flags & NETIF_VLAN_CANT_SUPPORT )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow: tx interface %s cannot support too many vlan tag\n", p_multifield_info->cfg0.vlan_info.vlan_info_auto.tx_ifname);
        }
        ...
        if( !p_multifield_info->cfg0.vlan_info.vlan_info_manual.is_vlan ) //rx no vlan tag, then no check is_vlan
            p_multifield_info->cfg0.vlan_info.vlan_info_manual.is_vlan_mask = -1;
        else //otherwise, need to check vlan tag
        ...
        if ( (tx_ifinfo->flags & NETIF_PHY_ATM) )
        {
            ...
            qid = ifx_ppa_drv_get_netif_qid_with_pkt(&fake_skb, tx_ifinfo->vcc, 1);
            if ( qid >= 0 )
                p_multifield_info->cfg0.queue_id = qid;
            else
                p_multifield_info->cfg0.queue_id = tx_ifinfo->dslwan_qid;
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: mapped queue id from %d to %d\n", old_qid, p_multifield_info->cfg0.queue_id);
        }
    }
...
int32_t ifx_ppa_add_multifield_flow( PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, int32_t *index, uint32_t flag)   //hook
{
    int32_t res;
    ...
    ppa_memset( &flow, 0, sizeof(flow) );
     ...
     if ( p_multifield_info->cfg0.vlan_info.bfauto )
    {
        if( (res = ifx_ppa_auto_learn_multifield_flow(p_multifield_info ) ) != IFX_SUCCESS )
        {
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_auto_learn_multifield_flow fail\n");
            return res;
        }
    }
    ...
    flow.flag = flag;
    res = ifx_ppa_drv_add_multifield_entry( &flow, 0);
     if( res == IFX_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: suceed to add below flow to index %d:\n", *index);
    }
    else
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_add_multifield_flow: fail to add below flow \n");
    }
...
int32_t ifx_ppa_del_multifield_flow( PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag )   //hook
{
     int32_t res;
     PPE_MULTIFILED_FLOW flow;
    ...
    flow.multifield_info = *p_multifield_info;
    flow.flag = flag;
    res = ifx_ppa_drv_del_multifield_entry( &flow, 0);
    if( res == IFX_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow: suceed to delete below flow:\n");
    }
    else
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow: fail to delete below flow \n");
    }
    *p_multifield_info = flow.multifield_info;
    ppa_mf_flow_display(p_multifield_info);
...
int32_t ifx_ppa_quick_del_multifield_flow( int32_t index, uint32_t flag)   //hook
{
    int32_t res=IFX_SUCCESS;
    PPE_MULTIFILED_FLOW flow;
    ...
    {
        int32_t num = ifx_ppa_get_multifield_max_flow(0);
        int32_t i;

        for( i = 0; i < num; i++ )
        {
            res = ifx_ppa_quick_del_multifield_flow( i, 0);
        }
    }
    ...
    {
        flow.entry= index;
        res = ifx_ppa_drv_del_multifield_entry_via_index( &flow, 0);
    }
    
    if( res == IFX_SUCCESS )
    {   
        if( index == -1 )
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow: succeed to delete all flows\n");
        else
            ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow: succeed to delete flow %d\n", index);
    }
    else
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow: fail to delete flow %d\n", index);

    return res;
}

int32_t ifx_ppa_get_multifield_flow( int32_t index, PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag )   //hook
{
    int32_t res;
    PPE_MULTIFILED_FLOW flow;
    ...
    flow.entry = index;
    flow.flag = flag;
    res = ifx_ppa_drv_get_multifield_entry( &flow, 0);

    if( p_multifield_info ) *p_multifield_info = flow.multifield_info;

    if( res == IFX_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_get_multifield_flow: suceed to get flow information %d\n", index);
        ppa_mf_flow_display(p_multifield_info);
    }
    else
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_get_multifield_flow: fail to get flow information %d\n", index);
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if ( ppa_copy_from_user( &cmd_info->mf_info, (void *)arg, sizeof(cmd_info->mf_info)) != 0 )
        return IFX_FAILURE;

    res = ifx_ppa_multifield_control( cmd_info->mf_info.enable_multifield, cmd_info->mf_info.flag);
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if( ifx_ppa_get_multifield_status( &enable, 0 )  != IFX_SUCCESS)
        return IFX_FAILURE;
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    ifx_ppa_add_multifield_flow( &cmd_info->mf_flow_info.flow, &cmd_info->mf_flow_info.index, cmd_info->mf_flow_info.flag );
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    cmd_info->count_info.count = ifx_ppa_get_multifield_max_flow( cmd_info->count_info.flag );
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if( ifx_ppa_get_multifield_flow( cmd_info->mf_flow_info.index, &cmd_info->mf_flow_info.flow, cmd_info->mf_flow_info.flag ) != IFX_SUCCESS )
    {
         ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_getl_multifield_flow fail\n");
        return IFX_FAILURE;
    }
    if ( ppa_copy_to_user( (void *)arg, (void *)&cmd_info->mf_flow_info, sizeof(cmd_info->mf_flow_info)) != 0 )
    {
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if( ifx_ppa_del_multifield_flow( &cmd_info->mf_flow_info.flow, cmd_info->mf_flow_info.flag ) != IFX_SUCCESS )
   {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_del_multifield_flow\n");
        return IFX_FAILURE;
    }
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if( ifx_ppa_quick_del_multifield_flow( cmd_info->mf_flow_info.index, cmd_info->mf_flow_info.flag ) != IFX_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_MFE, "ifx_ppa_quick_del_multifield_flow fail\n");
        return IFX_FAILURE;
    }
...
EXPORT_SYMBOL(ifx_ppa_multifield_control);
EXPORT_SYMBOL(ifx_ppa_get_multifield_status);
EXPORT_SYMBOL(ifx_ppa_get_multifield_max_flow);
EXPORT_SYMBOL(ifx_ppa_add_multifield_flow);
EXPORT_SYMBOL(ifx_ppa_del_multifield_flow);
EXPORT_SYMBOL(ifx_ppa_quick_del_multifield_flow);
